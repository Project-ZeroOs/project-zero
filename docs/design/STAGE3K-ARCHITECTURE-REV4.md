# Stage 3K — Storage / Filesystem Architecture Specification (Rev4)

**Status**: 🟢 APPROVED / READY TO FREEZE (Rev4)  
...
Stage 3K introduces the persistent storage primitive for Project Zero / ZeroOS.
...
Stage 3K establishes **ZeroFS**, a capability-native filesystem with:
...
- Double-indirect block pointers (deferred to Stage 3L volume expansion).
...
## 6. Exact Persistent Disk Layout & Volume Capacity (BLOCKER E Resolved)
...
## 8. Exact Journal Byte Layout & Checksum (BLOCKER 1 & 7 Resolved)
...
## 9. Transaction Capacity Bounds & Worst-Case Proof (BLOCKER 2 Resolved)

### 9.1 Transaction Limits
```rust
pub const MAX_JOURNAL_RECORDS: usize = 1;
pub const MAX_DATA_BLOCKS_PER_TX: usize = 8;     // 32 KiB data payload
pub const MAX_METADATA_BLOCKS_PER_TX: usize = 1; // 1 single-indirect block
pub const MAX_TOTAL_ALLOCATIONS_PER_TX: usize = 9; // 8 data + 1 indirect <= 10
pub const MAX_TOTAL_FREES_PER_TX: usize = 9;       // 8 data + 1 indirect <= 10
pub const MAX_TRANSACTION_BYTES: usize = 32_768; // 32 KiB
```

### 9.2 Mathematical Proof of Worst-Case Transaction Fit
In any single filesystem mutation:
1. **Case A: Direct Write**: Allocates $\le 8$ data blocks, 0 indirect blocks. Total allocations $= 8 \le 10$.
2. **Case B: Direct-to-Indirect Boundary**: Allocates $k$ direct data blocks, $8 - k$ indirect data blocks, and exactly $1$ single-indirect metadata block. Total allocations $= 8 + 1 = 9 \le 10$.
3. **Case C: Inside Indirect**: Allocates $\le 8$ indirect data blocks, 0 new indirect blocks. Total allocations $= 8 \le 10$.
4. **Case D: Create File**: Allocates 1 inode, 0 data blocks (or 1 directory data block if expanding directory). Total allocations $\le 2 \le 10$.
5. **Case E: Truncate / Delete**: Frees $\le 8$ data blocks $+ 1$ indirect block $= 9 \le 10$.

In all cases, the transaction requires $\le 9$ allocated blocks and $\le 9$ freed blocks. The journal's 10-block arrays (`allocated_blocks: [u32; 10]` and `freed_blocks: [u32; 10]`) strictly and provably guarantee that every valid transaction fits inside the 4096-byte journal block!

---

## 10. Commit Protocol, Multi-Transaction Semantics & Crash Guarantees (BLOCKER 3 Resolved)

### 10.1 Six-Step Commit Protocol
```text
Step 1: Write Journal INTENT Record
        - Populates DiskJournalBlock with state = Intent (1), op_type, old_inode_image,
          new_inode_image, allocated_blocks, dir_entry_copy, and CRC32.
...
Step 2: Write Data Blocks to Disk
        ...
        - Executes ATA CACHE FLUSH.
        - INVARIANT: The on-disk Inode Table still contains old_inode_image!
...
Step 4: Write Metadata Blocks to Disk
        - Writes new_inode_image into Inode Table on disk.
...
### 10.2 Transaction Atomicity vs Syscall Atomicity
...
## 11. Deterministic Crash Recovery Matrix

| Crash Point | Journal State | On-Disk Data | On-Disk Inode Table | Mount Recovery Action | Guarantee |
| :--- | :--- | :--- | :--- | :--- | :--- |
| Before Step 1 | `Free` (0) | Unchanged | Pre-TX | None. Clean mount. | Pre-transaction state |
| During Step 1 | Torn write | Unchanged | Pre-TX | Checksum fails. Reset journal to `Free`. | Pre-transaction state |
| After Step 1, Before Step 3 | `Intent` (1) | Partially written | Pre-TX (`old_inode_image`) | **Physical Rollback**: Write `old_inode_image` to Inode Table. Deallocate `allocated_blocks` in bitmap. Clear journal to `Free`. | Strict pre-transaction state; zero orphan blocks |
| During Step 3 (Commit Flush) | Torn write | Fully written | Pre-TX (`old_inode_image`) | If CRC32 invalid: **Rollback** as Intent. If CRC32 valid: **Rollforward** as Committed. | Either pre-TX or post-TX state |
| After Step 3, Before Step 6 | `Committed` (2) | Fully written | Partially updated | **Physical Rollforward**: Write `new_inode_image` to Inode Table; write `dir_entry_copy` to Directory Block; set `allocated_blocks` in bitmap; update superblock; clear journal to `Free`. | Strict post-transaction state; zero data loss |
...
| Corrupt Magic / Checksum | Invalid | Any | Dirty | **Fail-Closed**: Mount aborted with `Err(FsError::CorruptJournal)`. | Fail-Closed |
| Invalid Inode / Block Pointer | Any | Any | Dirty | **Fail-Closed**: Mount aborted with `Err(FsError::InvalidMetadata)`. | Fail-Closed |

---

## 12. StorageObject Structure & File Offset Semantics (BLOCKER 3 Resolved)

### 12.1 `StorageObject` Structure (48 bytes, 8-byte aligned)
...
### 12.2 Handle Duplication & Seek Offset Semantics
...
## 13. Capability Authority & Namespace Traversal

### 13.1 Directory Capability Requirement
...
### 13.2 Storage Rights Definition
```rust
pub mod cap_rights {
...
## 15. Global Lock Hierarchy & Acquisition Traces (BLOCKER 4 Resolved)

### 15.1 Global Monotonic Lock Hierarchy
```text
...
### 15.3 Exhaustive Lock Acquisition Traces (13 Operations)
...
## 16. ATA Bounded Polling & Device Failure Semantics (BLOCKER 6 Resolved)
...
## 17. MemBlockDevice vs Persistent Disk (Section 17 Resolved)
...
## 18. Reclamation & Lifecycle Ownership
...
## 19. Security & Untrusted Disk Validation
...
## 20. Zero-Heap Bounded Resources
...
## 21. Machine-Level Verification Suite (19 Tests: 3K-A through 3K-S)
...
## 24. Adversarial Review Classifications
...
## 25. Freeze Criteria & Final Verdict

All freeze criteria, exact byte tables, transaction capacity limits, recovery proofs, lock traces, and verification matrices are completely defined with zero open RED blockers.

# Stage 3N — SMP / Multi-Core Architecture (Rev4)

**Status:** 🟢 APPROVED / FROZEN (Stage 3N Rev4 + Rev3 Protocol Amendments)  
...
Rev4 resolves the final architectural issues identified in the Rev3 review:
1. **Process Activation Prohibition (`I-SMP-ASPACE-2`):** Formally closes the activation-vs-exit race by mandating that `switch_address_space_locked` checks the authoritative Process lifecycle state under `aspace.lock`. Processes in `Terminating`, `Zombie`, `Reclaiming`, or `Free` states can never be newly activated on any CPU.
2. **Hardware-Safe Quiescence Guarantee (`I-SMP-ASPACE-3`):** Eliminates premature page-table reclamation risk. An unresponsive CPU cannot permit memory destruction merely because it is marked `Failed`. In accordance with ZeroOS fail-closed safety principles, an unquiesced core at exit rendezvous triggers a fail-closed kernel panic (`CRITICAL: Process exit rendezvous timeout — unquiesced CPU`), mathematically preventing physical memory use-after-free.
3. **Unified Monotonic Generation Namespace:** Unifies page-table mutations and process exit rendezvous into a single monotonic $u64$ generation namespace per `AddressSpace`. Exact ACK matching ($== gen$) is required; generation wraparound is strictly forbidden (`u64::MAX` panics fail-closed).
4. **Deterministic All-Cores-Busy Wakeup Policy:** Formulates the exact deterministic preemption rule for `Critical` and `High` wakeups when every core in the system is currently executing work.
5. **Comprehensive 46-Test Verification Suite:** Adds machine tests `3N-AP` through `3N-AT` covering activation rejection, exit-vs-activation concurrency, unquiesced CPU safety, unified generation ACK matching, and all-busy preemption semantics.
...
## 5. Multi-Core Scheduler Ownership & Deterministic Preemption (`I-SMP-SCHED-1`)
...
> 4. **Wakeup Routing (Idle or Partially Busy):** Awakened threads preferentially route to their home core if idle or executing lower priority; otherwise routed to the lowest-numbered idle core.
> 5. **Deterministic Preemption Under All-Cores-Busy:** When an awakened thread of priority $P \in \{\text{Critical}, \text{High}\}$ cannot find an idle core:
>    - **Case A (Home Core Preemptible):** If the home core is currently executing a thread of priority $P_{home} < P$, the thread is enqueued on the home core's runqueue, and `IPI_VECTOR_RESCHEDULE` (252) is dispatched to the home core.
>    - **Case B (Remote Preemption):** If $P_{home} \ge P$, the scheduler scans all other online cores $C$ to find the core with the lowest currently running priority $P_{min}$. If $P_{min} < P$, the thread is migrated to core $C_{min}$, enqueued on $C_{min}$'s runqueue (under `min/max` lock order), and `IPI_VECTOR_RESCHEDULE` (252) is dispatched to $C_{min}$.
>    - **Case C (No Preemption Possible):** If all online cores are currently executing threads of priority $\ge P$, the thread is enqueued on its home core (FIFO tail for priority $P$) without sending a reschedule IPI. It executes when the home core reaches its next scheduling boundary.
...
pub struct AddressSpace {
    ...
    pub pid: ProcessId,                    // Owning process ID
    pub active_cpu_mask: AtomicU32,        // Bit i = 1 if CPU i currently has PML4 in CR3
    pub lock: SchedLock,                   // Level 11 lock: serializes activation, mutation, exit
    pub shootdown_generation: AtomicU64,   // Unified monotonic generation counter (starts at 1)
}
```
...
### 7.3 Unified Monotonic Generation Namespace
Page-table mutations and process exit rendezvous share **one unified monotonic $u64$ generation namespace** per `AddressSpace`:
- Every mutation increments `shootdown_generation` by 1.
- Every exit rendezvous increments `shootdown_generation` by 1.
- Generation wraparound is strictly forbidden: if `gen == u64::MAX`, the kernel panics fail-closed (`GenerationExhausted`).
- Remote CPUs acknowledge exact generation $gen$; acknowledging $G > gen$ or $G < gen$ does *not* satisfy the barrier.

### 7.4 Exact Atomic Activation Protocol (`switch_address_space_locked`)
...
3. If `old_aspace` is not null:
   - Acquire `old_aspace.lock` (Level 11).
   - Clear `old_aspace.active_cpu_mask.clear(cpu_id)`.
   - Release `old_aspace.lock`.
4. If `new_aspace` is null:
   - Load `CR3 = MASTER_KERNEL_PML4`.
   - Write `PER_CPU_STATE[cpu_id].active_pid = 0`.
   - Return.
5. Acquire `new_aspace.lock` (Level 11).
6. **Authoritative Process Lifecycle Check (`I-SMP-ASPACE-2`):**
   - Query owning process state: `proc_state = get_process_state(new_aspace.pid)`.
   - **If `proc_state != ProcessState::Active` (i.e. Terminating, Zombie, Reclaiming, Free):**
     - Release `new_aspace.lock`.
     - Reject activation. Load fallback: `CR3 = MASTER_KERNEL_PML4`.
     - Write `PER_CPU_STATE[cpu_id].active_pid = 0`.
     - Mark $T_2$ as `Terminated` and reschedule.
     - Return.
7. Read `gen = new_aspace.shootdown_generation.load(Relaxed)`.
8. Load hardware register: `CR3 = new_aspace.pml4_root`.
9. Set `new_aspace.active_cpu_mask.set(cpu_id)`.
10. Write `PER_CPU_STATE[cpu_id].tlb_ack_gen = gen`.
11. Write `PER_CPU_STATE[cpu_id].active_pid = new_aspace.pid`.
12. Release `new_aspace.lock`.

### 7.5 Exact Atomic Mutation Protocol (`unmap_page` / `protect_page`)
...
### 7.6 Deadlock-Free Proof of `aspace.lock`
...
## 8. Process & Address-Space SMP Lifecycle (`I-SMP-ASPACE-1`, `I-SMP-ASPACE-2`, `I-SMP-ASPACE-3`)

### 8.1 Invariant `I-SMP-ASPACE-1` (Coherent Process Exit Rendezvous)
> **Invariant `I-SMP-ASPACE-1`:**
> AddressSpace reclamation is forbidden until every CPU that was identified as an active executor has completed the rendezvous and acknowledged departure with matching exit generation.

### 8.2 Invariant `I-SMP-ASPACE-2` (Activation Prohibition on Non-Active Processes)
> **Invariant `I-SMP-ASPACE-2`:**
> A Process in `Terminating`, `Zombie`, `Reclaiming`, or `Free` state cannot be newly activated on any CPU. `switch_address_space_locked` must consult the authoritative Process lifecycle state under `aspace.lock` and reject activation if the owning Process is no longer `Active`.

### 8.3 Invariant `I-SMP-ASPACE-3` (Hardware Quiescence Guarantee & Fail-Closed Panic)
> **Invariant `I-SMP-ASPACE-3`:**
> An unresponsive CPU does not permit AddressSpace reclamation merely because its lifecycle state is marked `Failed`. AddressSpace reclamation requires mathematical proof that no CPU can execute with the AddressSpace loaded in CR3. If an active CPU fails to acknowledge departure within `MAX_RENDEZVOUS_POLL_TICKS`, the kernel triggers an immediate fail-closed panic:
> ```text
> CRITICAL: Process exit rendezvous timeout — unquiesced CPU {}
> ```
> This prevents physical memory frames from ever being reclaimed while a remote core can potentially execute with the page table loaded.

### 8.4 Exact Remote-CPU Process Exit Rendezvous State Machine
...
Initiating Core (CPU 0)                           Remote Active Core (CPU 1)
 ...
 │     (Under I-SMP-ASPACE-2, no core can ever      │
 │      newly activate process P from this point)   │
 │                                                  │
 ...
 │                                                  ├─ switch_address_space checks process state,
 │                                                  │  switches CR3 to MASTER_KERNEL_PML4
 ...
 │                                                  │
 ├─ 5. Fail-Closed Quiescence Guard (I-SMP-ASPACE-3)│
 │     If any core fails to evacuate within 100 ms: │
 │       Kernel Panics Fail-Closed:                 │
 │       "CRITICAL: Process exit rendezvous timeout"│
 │       (Strictly prevents memory corruption/UAF)  │
 │                                                  │
 ├─ 6. Reclamation:                                 │
...
## 11. Extended Machine Verification Suite (46 Tests)

Stage 3N defines **46 sequential bare-metal machine verification tests**:
...
### Rev3 Boundary Tests (`3N-AI` through `3N-AO`)
...
- `3N-AM`: **Remote CPU Rendezvous Timeout / Freeze Fallback:** Unresponsive core rendezvous timeout validation.
...
### Rev4 Boundary & Safety Tests (`3N-AP` through `3N-AT`)
- `3N-AP`: **Activation Rejected After Process $\to$ Terminating (`I-SMP-ASPACE-2`):** Verifies that `switch_address_space_locked` rejects activation of a process marked `Terminating`, loading `MASTER_KERNEL_PML4` instead.
- `3N-AQ`: **Exit Race: Activation vs Termination:** Concurrent execution where Core 0 marks process `Terminating` while Core 1 attempts `switch_address_space_locked`; proves Core 1 is rejected and cannot join `active_cpu_mask`.
- `3N-AR`: **Failed/Unresponsive CPU Memory Safety (`I-SMP-ASPACE-3`):** Confirms address space frames are NEVER reclaimed when a CPU fails to acknowledge exit rendezvous, enforcing fail-closed panic.
- `3N-AS`: **Unified Generation Monotonicity & ACK Matching:** Validates unified $u64$ generation namespace across mutations and exit rendezvous, verifying exact ACK matching and rejection of reused/stale generation tags.
- `3N-AT`: **All-Cores-Busy Preemption Semantics:** Validates deterministic preemption behavior when an awakened `Critical` thread preempts the lowest-priority core among all busy cores.

---

```text
ARCHITECTURE STATUS:
🟡 PROPOSED — AWAITING REVIEW (Rev4)
```
